...
实际上r = r’ * in_height / in_width. 和你下面的式子等价。当然两种方法都可以生成正确的锚框，就是逻辑和代码有出入。
    """
    w = torch.cat((size_tensor * torch.sqrt(ratios_tensor[0]), size_tensor[0] * torch.sqrt(
        ratios_tensor[1:]))) * in_height / in_width  # 此处乘以in_height可以理解，除以in_width似乎是为了归一化，但为什么h没有归一化？
    ...
    anchor_manipulations = torch.stack((-w, -h, w, h)).T.repeat(in_height * in_width,
                                                                1) / 2  # 为了后面计算左上、右下坐标；每一行都是[-w,-h,w,h]
    out_grid = torch.stack([shift_x, shift_y, shift_x, shift_y], dim=1).repeat_interleave(boxes_per_pixel,
                                                                                          dim=0)  # 每一行变成了[x,y,x,y]
...
def box_iou(boxes1, boxes2):
    ...
    inter_upperlefts = torch.max(boxes1[:, None, :2], boxes2[:,
                                                      :2])  # None升维——size[5,2]升为size[5,1,2]，为的是box之间两两能比较，计算结果为torch.Size([5, 5, 2]) 即5*5个box的计算结果
...
def multibox_detection(cls_probs, offset_preds, anchors, num_threshold=.5, pos_threshold=.0099999999):
    """
    :param cls_probs: [batch, classes类别数, anchors数目]
    :param offset_preds: [batch, anchors数 * 4]
    :param anchors:[batch, anchors数, 4]
    :param num_threshold:
    :param pos_threshold:
    :return:
    """
    ...
    for i in range(batch_size):
        ...
        conf, class_id = torch.max(cls_prob[1:], dim=0)     # [1:]因为索引第0个类别是背景类
        predicted_bb = offset_inverse(anchors, offset_pred)
        keep = nms(predicted_bb, conf, num_threshold)   # 保留的预测索引框

        all_idx = torch.arange(num_anchors, dtype=torch.long, device=device)    # 所有anchors的索引
        combined = torch.cat((keep, all_idx))       # keep全包含在all_idx中，后面统计索引出现的次数就知道谁是不是keep
        uniques, counts = combined.unique(return_counts=True)
        non_keep = uniques[counts == 1]     # keep和all_idx拼接的，只出现1次的，就是没保留的
        all_id_sorted = torch.cat((keep, non_keep))     # keep和non_keep的预测框索引依次排好
        class_id[non_keep] = -1
        class_id = class_id[all_id_sorted]      # class_id也按照keep、non_keep索引顺序排好
        conf, predicted_bb = conf[all_id_sorted], predicted_bb[all_id_sorted]       # 都按照keep、non_keep索引顺序排好
        below_min_idx = (conf < pos_threshold)      # 小于pos_threshold就是背景类
        class_id[below_min_idx] = -1                # 小于pos_threshold的类别设为-1
