...
class Inception(nn.Module):
    ...
    def __init__(self, in_channels, c1, c2, c3, c4):
        ...
        self.p1 = nn.Sequential(
            nn.Conv2d(in_channels, c1, kernel_size=1), BatchNorm(4, c1), nn.ReLU()
        )
        self.p2 = nn.Sequential(
            nn.Conv2d(in_channels, c2[0], kernel_size=1), BatchNorm(4, c2[0]), nn.ReLU(),
            nn.Conv2d(c2[0], c2[1], kernel_size=3, padding=1), BatchNorm(4, c2[1]), nn.ReLU()
        )
        self.p3 = nn.Sequential(
            nn.Conv2d(in_channels, c3[0], kernel_size=1), BatchNorm(4, c3[0]), nn.ReLU(),
            nn.Conv2d(c3[0], c3[1], kernel_size=5, padding=2), BatchNorm(4, c3[1]), nn.ReLU()
        )
        self.p4 = nn.Sequential(
            nn.MaxPool2d(kernel_size=3, padding=1, stride=1),
            nn.Conv2d(in_channels, c4, kernel_size=1), BatchNorm(4, c4), nn.ReLU()
        )
...
class GoogLeNet(nn.Module):
    def __init__(self, three_block_arch):
        ...
        self.block_1 = nn.Sequential(
            nn.Conv2d(1, 64, kernel_size=7, stride=2, padding=3), BatchNorm(4, 64), nn.ReLU(),    # 高宽刚好减半。应该是padding和kernel_size刚好抵消，只stride起到控制减半作用了
            nn.MaxPool2d(kernel_size=3, stride=2, padding=1)    # 高宽刚好减半
        )
        # build block two--reduced by a factor of 2
        self.block_2 = nn.Sequential(
            nn.Conv2d(64, 64, kernel_size=1), BatchNorm(4, 64), nn.ReLU(),
            nn.Conv2d(64, 192, kernel_size=3, padding=1), BatchNorm(4, 192), nn.ReLU(),
